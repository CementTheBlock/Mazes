...
impl Node {
    ...
    fn print_node(&self) {
        println!("{4}\n{0}\n{1}\n{2}\n{3}",
                 opening_to_string(self.up),
                 opening_to_string(self.down),
                 opening_to_string(self.left),
                 opening_to_string(self.right),
...
fn update_two_nodes(maze: Maze, cursor_one: Cursor, cursor_two: Cursor, dir: Direction) -> Maze {
    ...
    ret_maze.nodes[cursor_one.index] =
        update_node_in_dir(*ret_maze.get(cursor_one), dir, &cursor_two);
    ret_maze.nodes[cursor_two.index] =
        update_node_in_dir(*ret_maze.get(cursor_two), reverse_dir(dir), &cursor_one);
    ret_maze
}
